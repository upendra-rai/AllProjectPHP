						...
						<section class="section_card" id="table_section">
							  ...
							  <div class="panel-body " style="padding-top:0px;">
								   <table class="table mb-none" id="table" data-toggle="table" data-pagination="true" style="border-bottom: 1px solid #424351;">
									...
									<tbody>
										  <?php $i = 1; foreach($banner_list as $row){ ?>
                          <tr>
														  <td><?php echo $i++; ?></td>
                                                ...
                                                <td>
                                                    <a 
                                                       href="<?php echo base_url(); ?>/management/edit_banner/<?php echo $row->banner_id; ?>"> <button type="button"   style=" width:100%;"><i class="fa fa-pencil"></i></button>
					...
					<section class="section_card floating_form">
						 ...
						 <div class="panel-body" style="padding-left:0px; padding-right:0px;  background-color:#27293d; padding-top:15px; padding-bottom:0px;">
										...
										<div class="form-group">
											<div class="col-md-12">
												<input type="text" class="form-control" name="banner_title" value="<?php if(isset($selected_banner[0])){ echo $selected_banner[0]->banner_title; } ?>"  required><span class="placeholder">Banner Tittle</span>
											</div>
										</div>
								     </div>
                                   ...
                                   <div class="col-md-6">
										<div class="form-group">
											<div class="col-md-12">
												<input type="text" class="form-control" name="banner_subtitle" value="<?php if(isset($selected_banner[0])){ echo $selected_banner[0]->banner_subtitle; } ?>"  required><span class="placeholder">Banner SubTittle</span>
											</div>
										</div>
								     </div>
									 ...
									 <div class="col-md-12">
										  <div class="form-group">
											   <div class="col-md-12">
												 ...
												 <label for="file" id="img_label" style="width:150px; height:150px; border:1px solid #424351; text-align:center;  font-size:25px;"><?php if(isset($selected_banner[0])){ ?> <img src="<?php echo base_url(); ?>uploads/app_banner_image/<?php echo $selected_banner[0]->banner_img; ?>" data-img_name="" alt="" style="width:100% ; height:100%;"/><input type="hidden" name="banner_img" value="<?php echo $selected_banner[0]->banner_img; ?>" /><button type="button" class="btn" id="temprory_remove_image" style="position:absolute; background-color:#ff4747; color:#ffffff; margin-top:-2px;">X</button>  <?php }else{ ?> <i class="fa fa-globe" style="margin-top:60px; margin-right:5px;"></i><span style="font-size:12px; ">Browse</span> <?php  } ?>  </label>
											   </div>
												 <!-- last uploaded image name -->
											   <input type="hidden" name="last_uploaded_image_name" value="<?php if(isset($selected_banner[0])){ echo $selected_banner[0]->banner_img; }?>">
										 </div>
									</div>
									<div class="col-md-6">
											<div class="form-group">
									...
									<div class="col-md-6">
											<div class="form-group">
												<div class="col-md-12">
												<a href="<?php echo base_url(); ?>management/banner">	<button type="button" class="btn-transparent btn-red"  style=" width:100%;">Cancel</button></a>
												</div>
											</div>
						      </div>
									</form>
    ...
    document.getElementById('file').addEventListener('change', (e) => {
        ...
        success(result) {
           ...
           formData.append('img_folder_name', img_folder_name);
            $.ajax({
		    	url: '<?php echo base_url(); ?>/management/upload_banner_image',
		    	method:"POST",
		    	data:formData,
		    	contentType:false,
		    	cache:false,
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/management/delete_banner',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
										if(data === 'success'){
