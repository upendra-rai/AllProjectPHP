						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:0px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
														...
														Commission List <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> 
														<i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i>
														
														<a href="<?php echo base_url(); ?>/master/add_commission"type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add Commission</a>
													</h2>
									    </div>
								</header>
							   <div class="panel-body " style="padding-top:0px;">
								   <table class="table mb-none" id="table" data-toggle="table" data-pagination="true" style="border-bottom: 1px solid #424351;">
									...
									<tbody>
										  <?php $i = 1; if(isset($commission_list)){ foreach($commission_list as $row){ ?>
                          <tr>
														  <td><?php echo $i++; ?></td>
                          	                            <td><?php echo $row->from_amount; ?></td>
                                                        <td><?php echo $row->to_amount; ?></td>
                                                        <td><?php echo $row->commission_percent; ?></td>
                                                        <td><?php echo $row->commission_rs; ?></td>
                                                       
												       <td><?php echo $row->commission_create_at; ?></td>
                               <td>
                    <a 
                       href="<?php echo base_url(); ?>/master/edit_commission/<?php echo $row->commission_id; ?>"> <button type="button"   style=" width:100%;"><i class="fa fa-pencil"></i></button>
                         
                   </a>
                      </td>
                                               
                              <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->commission_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button>
                              </td>
                          </tr>
										<?php }} ?>
									</tbody>
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/master/delete_commission',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
										if(data === 'success'){
