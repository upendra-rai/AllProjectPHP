								 ...
								 <header class="panel-heading" style="padding:0px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
														...
														Product <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> 
														
														
														<a href="<?php echo base_url(); ?>/management/add_product"type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add Product</a>
													</h2>
									    </div>
								</header>
								   ...
								   <table class="table mb-none" id="table" data-toggle="table" data-pagination="true" style="border-bottom: 1px solid #424351;">
									...
									<tbody>
                                        <?php function sum($num) { 
										  ...
										  <?php $i = 1;  if(isset($number_list)){ foreach($number_list as $row){ ?>
                                                     <tr>
														  <td><?php echo $i++; ?></td>
                                                        <td><?php echo $row->product_vanity_no; ?></td>
                                                         <td><?php  $number_sum =  sum($row->product_vanity_no); echo $number_sum; echo '='; $sum_value = sum(json_encode($number_sum)); echo $sum_value; ?></td>
                                                        ...
                                                        <td><?php echo $row->vendor_rate; ?></td>
                                                         ...
                                                         <td><?php echo $row->user_like; ?></td>
                                                         <td><?php echo $row->user_view; ?></td>
                                        ...
                                        <td>
                                         ...
                                         <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->my_product_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button>
                                              </td>
                          </tr>
										<?php }} ?>
									</tbody>
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/management/delete_product',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
										if(data === 'success'){
