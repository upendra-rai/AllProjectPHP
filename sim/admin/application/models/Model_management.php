...
/*##############################################Category########################################################################################################################################################################*/
   ...
   public function edit_subcategory($edit_id,$category_id,$sub_category_name,$sub_category_desc){
    ...
    public function upload_product($filepos){
        ...
        'product_vanity_no' =>$data[0],
        'product_rate' =>$data[0],
        'vendor_id' =>$data[0],
        'vendor_rate' =>$data[0],
        'product_cod_available' =>$data[0],
        'discount_percentage' =>$data[0],
        'flat_discount' =>$data[0],
        
         );
         if($this->db->insert('product',$filedata)){
        echo 'success';
          redirect(base_url().'/management/number_details');
...
public function vendor_details($name,$mobile_no,$status){
    ...
    public function pirority_list(){
        $this->db->select('*');
		$this->db->from('vendor_priority');
        $this->db->join('vendor','vendor.vendor_id=vendor_priority.vendor_id');
		$data = $this->db->get();
		return $data->result();
    }
