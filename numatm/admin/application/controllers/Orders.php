...
//=========********===========********========******========//
  public function dispatched_orders(){
    ...
    public function upc_details(){
		...
		$data["pending_list"] = $this->model_orders->pending_upload();
		$this->load->view('status/upc_details');
	}
    
     public function add_upc(){
         $product_id= $this->uri->segment(3);
        if($this->input->post('submit')){
            $service_provider =$this->input->post('operator_code');
            $circle =$this->input->post('circle_code');
            $upc_code =$this->input->post('upc_code');
            $generation_date=$this->input->post('generation_date');
            $expiry_date=$this->input->post('expiry_date');
            
        //echo  $service_provider.$circle.$upc_code.$generation_date.$expiry_date;
  
    $this->model_orders->add_upc($product_id,$service_provider,$circle,$upc_code,$generation_date,$expiry_date);
 
    }else{
            $data['selected_product'] =$this->model_orders->selected_product($product_id);
            echo $product_id;
            $data['active_menu'] = "status";
		$data['active_submenu'] = "upc_details";
      $this->load->view('/status/add_upc',$data);    
  }  
    ...
    public function get_operator_code(){
        if(isset($_POST['operator_code'])){
           ...
           // echo $operator_code;
            $data['operator_code'] = $this->model_orders->get_operator_coder($operator_code);
            echo json_encode($data);
        }
    }
     public function get_circle_code(){
        if(isset($_POST['circle_code'])){
            $circle_code = $_POST['circle_code'];
             $data['circle_code'] = $this->model_orders->get_circle_code($circle_code);
            echo json_encode($data);
        } 
    }
