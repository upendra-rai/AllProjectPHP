...
public function product_details(){
    ...
    public function update_vendor_priority(){
        
        if(isset($_POST['arr'])){
            
            $arr =  $_POST['arr'];
           /// print_r($arr);
           $this->model_management->update_vendor_priority($arr);
    ...
    public function test(){
       ...
       $image=$this->input->post('image');
            ...
            echo $name.$image;
    //$this->model_management->add_test($name,$image);
	  ...
	  if($_FILES["file"]["name"] != "")
      {
	   ...
	   $location = 'uploads/'.$img_folder_name.'/'.$name;
	   move_uploaded_file($_FILES["file"]["tmp_name"], $location);
	   echo '<img src="'.base_url().$location.'" data-img_name="'.$name.'" alt="User" style="width:100% ; height:100%;"/><input type="hidden" name="image" value="'.$name.'" /><button type="button" class="btn" id="inlink_img_bt" data-unlink_img_src="'.$location.'" style="position:absolute; background-color:#ff4747; color:#ffffff; margin-top:-2px;">X</button>';
