						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:0px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													 ...
													 Category List <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> <?php  if(isset($product_subcategory[0])){ echo $product_subcategory[0]->product_category_name;  } ?>
													</h2>
									    </div>
								</header>
							  <div class="panel-body " style="padding-top:0px;">
								   <table class="table mb-none" id="table" data-toggle="table" data-pagination="true" style="border-bottom: 1px solid #424351;">
									...
									<tbody>
				    <?php $i = 1; if(isset($category_list)){ foreach($category_list as $row){ ?>
                          <tr>
								          <td><?php echo $i++; ?></td>
                                      <td><?php echo $row->category_name; ?></td>								
				  <td>
                    <a 
                       href="<?php echo base_url(); ?>/management/edit_category/<?php echo $row->my_category_id; ?>"> <button type="button"   style=" width:100%;"><i class="fa fa-pencil"></i></button>
                         
                   </a>
                </td>
                <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->my_category_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button>
                    </td>
                          </tr>
										<?php } }?>
									</tbody>
						...
						<section class="section_card" id="table_section">
									...
									<div class="col-md-6">
										<div class="form-group">
											<div class="col-md-12">
												<input type="text" class="form-control" name="category_name" value="<?php if(isset($selected_category[0])){ echo $selected_category[0]->category_name; } ?>"  required><span class="placeholder"> Category Name</span>
											</div>
										</div>
								     </div>
                            ...
                            <div class="tinymce-area mg-b-15"  style="display:none;">
                                <div class="container-fluid">
                                   ...
                                   <div class="col-md-6">
										<div class="form-group">
											<div class="col-md-12">
												<input type="text" class="form-control" name="category_desc" value="<?php if(isset($selected_category[0])){ echo $selected_category[0]->category_desc; } ?>" ><span class="placeholder"> Category Description</span>
											</div>
										</div>
								     </div>
                                    ...
                                    <div class="col-md-12">
										  <div class="form-group">
											   <div class="col-md-12">
												 <input type="file" name="file" id="file" accept="image/*" value="" data-img_folder_name="category_img" style="display:none;"/>
												 <label for="file" id="img_label" style="width:150px; height:150px; border:1px solid #424351; text-align:center;  font-size:25px;"><?php if(isset($selected_category[0])){ ?> <img src="<?php echo base_url(); ?>uploads/category_img/<?php echo $selected_category[0]->category_img; ?>" data-img_name="" alt="" style="width:100% ; height:100%;"/><input type="hidden" name="image_name" value="<?php echo $selected_category[0]->category_img; ?>" /><button type="button" class="btn" id="temprory_remove_image" style="position:absolute; background-color:#ff4747; color:#ffffff; margin-top:-2px;">X</button>  <?php }else{ ?> <i class="fa fa-globe" style="margin-top:60px; margin-right:5px;"></i><span style="font-size:12px; ">Browse</span> <?php  } ?>  </label>
											   </div>
												 <!-- last uploaded image name -->
											   <input type="hidden" name="last_uploaded_image_name" value="<?php if(isset($selected_category[0])){ echo $selected_category[0]->category_img; }?>">
										 </div>
									</div>        
									...
									<div class="col-md-6">
											<div class="form-group">
												<div class="col-md-12">
												<a href="<?php echo base_url(); ?>management/manage_category">	<button type="button" class="btn-transparent btn-red"  style="  width:100%; height:35px;">Cancel</button></a>
												</div>
											</div>
										</div>
						             </div>
    ...
    document.getElementById('file').addEventListener('change', (e) => {
        ...
        success(result) {
           ...
           formData.append('img_folder_name', img_folder_name);
            $.ajax({
		    	url: '<?php echo base_url(); ?>/management/upload_category_image',
		    	method:"POST",
		    	data:formData,
		    	contentType:false,
		    	cache:false,
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/management/delete_category',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
										if(data === 'success'){
