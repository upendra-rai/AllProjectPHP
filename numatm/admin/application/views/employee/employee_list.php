				...
				<section role="main" class="content-body" >
						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:0px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
														...
														Employee List <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> 
														<i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i>
														
														<a href="<?php echo base_url(); ?>/master/add_employee"type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add Employee</a>
													</h2>
									    </div>
								</header>
                          ...
                          <tr>
                                                ...
                                                <td><?php echo $row->profile_type_name; ?></td>
                                                <td><?php echo $row->employee_first_name.' '.$row->employee_last_name; ?></td>
                                                <td><?php echo $row->employee_contact_number; ?></td>
                                                <td><?php echo $row->employee_email_id; ?></td>  
												<td><?php echo $row->employee_create_at; ?></td>
                               <td>
                    <a 
                       href="<?php echo base_url(); ?>/master/edit_employee/<?php echo $row->employee_id; ?>"> <button type="button"   style=" width:100%;"><i class="fa fa-pencil"></i></button>
     ...
     $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/master/delete_employee',
								  method: 'POST',
									data:{del_id: del_id},
