...
class team extends CI_Controller {
    ...
    public function assign_colony(){
        $data['active_menu'] = "management";
        $data['active_submenu'] = "team";    
        
        $team_id = $this->uri->segment(3);
        $agent_name = $this->uri->segment(4);
        
        if($this->input->post('submit') != ""){
            
            $assign_colony = $this->input->post('assign_colony');
           
            echo $assign_colony;
            
            $data["agent_selected_colony"] = $this->model_team->assign_colony($team_id,$assign_colony);
            
        }else{
         
        $data['agent_name'] = $agent_name;     
        $data['selected_agent'] = $team_id;    
        $data["agent_selected_colony"] = $this->model_team->agent_selected_colony($team_id);
        $data["select_colony"] = $this->model_team->select_colony();    
        $this->load->view('team/assign_colony',$data);
