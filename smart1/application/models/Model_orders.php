...
public function order_accept_or_reject($order_id,$action){
    ...
    if($this->db->update('online_orders')){
        ...
        if($action == 'placed'){
            ...
            $cus_name = json_decode($mydata->result()[0]->delivery_details)[0]->name;
            $mobile_no = json_decode($mydata->result()[0]->delivery_details)[0]->mobile_no;
            $paid_amount = $mydata->result()[0]->paid_amount;
                                    ...
                                    // ******** send sms *******//
									//**************************//
                                 	$url = base_url()."send_verify_msg.php?mobile_no=".$mobile_no."&name=".$cus_name."&transaction_amount=".$paid_amount."&template=place_order"; 
		                            $cSession = curl_init(); 
