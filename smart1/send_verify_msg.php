...
ini_set('display_errors', 0);
  ...
  $client_url = 'https://smart.avsprimetechnology.com'; 
  
  $company = "SMART";
  $ending_line = "Team SMART";
  
    $msg = '';
  
  
        $cSession = curl_init(); 
        curl_setopt($cSession,CURLOPT_URL,$client_url."/e_commerce_app_interface/check_sms_balance/"); 
  ...
  if($template == 'ragistration'){
	...
	//$msg = "Dear ".$name.", Welcome to ".$company.". Your membership card No. is ".$card_no.". Please download our app to use the services. Best wishes.".$ending_line." AVS Prime Technologies LLP";
      ...
      $msg =  "Dear ".$name.",Welcome to ".$company.".Your account has been created successfully, Please download our app to use the services.Regards,x AVS Prime Technologies LLP";
 ...
 if($template == 'place_order'){
 
    $msg = "Dear ".$name.",Your order for Rs. ".$transaction_amount." has been placed successfully.Thank You,".$ending_line." AVS Prime Technologies LLP";
 }
  ...
  if($template == 'terminate'){
   ...
   if($template == 'otp'){
	   ...
	   $msg = "OTP for make Transaction on ".$company." is ".$otp_number." & valid till 3minut, Don't share it.Regards,".$ending_line." AVS Prime Technologies LLP";
        ...
        if($result){
			...
			if($result->ErrorMessage == 'Done'){
				...
				$cSession = curl_init(); 
				curl_setopt($cSession,CURLOPT_URL,$client_url."/e_commerce_app_interface/insert_sended_msg/"); 
				curl_setopt($cSession,CURLOPT_RETURNTRANSFER,true);
				curl_setopt($cSession,CURLOPT_HEADER, false); 
				$check_sms=curl_exec($cSession);
				//echo json_encode($result);
