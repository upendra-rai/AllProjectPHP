						...
						<section class="section_card" id="table_section">
							  ...
							  <div class="panel-body " style="padding-top:0px;">
                                  ...
                                  <form class="form-horizontal form-bordered" action="" method="post">
                                       <div class="col-md-2">
											<div class="form-group" style="display: <?php if($this->session->userdata('user_role') != 'master admin'){echo 'none';} ?>">
												
                                                <select class="form-control" name="outlet" id="outlet" <?php if($this->session->userdata('outlet_id')){ echo 'disabled'; } ?> >
														<option value="">Outlet</option>
														<?php foreach($outlet_list as $row){ ?>
														<option value="<?php echo $row->outlet_id ?>" <?php if(isset($selected_outlet)){ if($selected_outlet == $row->outlet_id){ echo 'selected'; }} ?> <?php if($this->session->userdata('outlet_id')){ if($this->session->userdata('outlet_id') == $row->outlet_id){ echo 'selected'; }} ?>  ><?php echo $row->outlet_name; ?></option>
													 <?php } ?>
													</select>
										 ...
										 <div class="form-group">
											 <div class="col-md-12">
												 <select class="form-control" name="subcategory" >
													  <option value="" class="first_option">Sub Category</option>
													 <?php foreach($subcategory_list as $row){ ?>
													 <option value="<?php echo $row->product_sub_category_id ?>" <?php if(isset($selected_subcategory)){ if($selected_subcategory == $row->product_sub_category_id){ echo 'selected'; }} ?> class="option_<?php echo $row->product_category_id; ?>"><?php echo $row->product_sub_category_name; ?></option>
													<?php } ?>
												 </select>
											 </div>
										 </div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<div class="col-md-12">
												<input type="text" class="form-control" name="product_name" value="<?php if(isset($product_selected)){ echo $product_selected; } ?>" placeholder="Product Name" >
											</div>
										</div>
								    </div>
									   	...
									   	<tr>
										     ...
										     <th>Image</th>
											   <th>Product Name</th>
												 ...
												 <th>Price</th>
                                             <th>GST</th>
											   <th class="hidden-phone">Alert Qty</th>
											   <th>Edit</th>
                          ...
                          <tr>
                                                          ...
                                                          <td><?php if($row->product_stock_qty > 0){ echo +$row->product_stock_qty; }else{ echo '<span style="color:#ff4748">0</span>'; } ?></td>
														  <td><?php echo $row->product_unit; ?></td>
															<td><?php echo $row->product_unit_price; ?></td>
                              
                                                          	<td><?php echo $row->sgst+$row->cgst; ?> %</td>
															<td><?php echo $row->stock_alert_qty; ?></td>
...
$(document).on('change','select[name=category]',function(){
			...
			if(category_id !== ''){
				var op = 'option_'+category_id;
                
                $('select[name=subcategory] option').hide();
                 $('select[name=subcategory] option.first_option').show();
                $('select[name=subcategory] option.'+op).show();
