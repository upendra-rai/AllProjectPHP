								 ...
								 <header class="panel-heading" style="padding:10px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													...
													<a href="<?Php echo base_url(); ?>account/add_money"><button type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">+ Add </button></a>
									    </div>
								</header>
								   ...
								   <table class="table mb-none" id="table" data-toggle="table" data-pagination="true" style="border-bottom: 1px solid #424351;">
									...
									<tbody>
										  <?php $i = 1; foreach($list as $row){ ?>
                                                  <tr>
														  <td><?php echo $i++; ?></td>
														  <td><?php echo $row->money_transfer_date; ?></td>

														  
                          	                               <td><?php echo $row->from_account_name; ?></td>
                          	                               <td><?php echo $row->to_account_name; ?></td>
                          	                               <td><?php echo $row->amount; ?></td>                             
					          <td>
                                  <a 
                              href="<?php echo base_url(); ?>/account/edit_money/<?php echo $row->money_transfer_id; ?>"> <button type="button"   style=" width:100%;"><i class="fa fa-pencil"></i>
                               </a>
                              </td>
                      

                              <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->money_transfer_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button>
                              </td>
                                                 </tr>
										<?php } ?>
									</tbody>
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/account/delete_money',
								  method: 'POST',
									data:{vdelv: del_id},
									success:function(data){
										//alert(data);
