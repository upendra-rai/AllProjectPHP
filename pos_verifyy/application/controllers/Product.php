...
class product extends CI_Controller {
	...
	public function edit_product()
	{
      ...
      $last_uploaded_image_name =  $this->input->post('last_uploaded_image_name');
			 ...
			 $data["msg"] = $this->model_product->edit_product($id,$posted_category_id,$posted_subcategory_id,$product_name,$unit,$unit_price,$status,$img_name,$count_scroll_position,$gst,$short_discription,$discription,$mrp_price);
...
// list product section
    ...
    public function list_product()
	{
       ...
       // url segment
		...
		if($this->input->post('submit')){
           ...
           if(!$posted_subcategory_id){
            ...
            $data['product_selected'] = $product_name;    
			 ...
			 $data["product_list"] = $this->model_product->select_all_product($posted_category_id,$posted_subcategory_id,$product_name);
              $data["category_list"] = $this->model_product->select_category($category_id,$subcategory_id);
		     $data["subcategory_list"] = $this->model_product->select_subcategory($category_id,$subcategory_id);   
		...
		$data["product_list"] = $this->model_product->select_all_product($category_id,$subcategory_id,$product_name);
		$this->load->view('product/list_product', $data);
	}
	}
    ...
    public function upload_product(){
        ...
        if(isset($_POST['submit'])){
            ...
            while(($filepos=fgetcsv($handel,1000,','))!==false){
                ...
                echo "<pre>";
                if($c>0){
                $this->model_product->upload_product($filepos);
                }
                $c=$c+1;
            }
