						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:10px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													...
													<a href="<?Php echo base_url(); ?>account/add"><button type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">+ Add </button></a>
									    </div>
								</header>
                         ...
                         <tr>
		                           ...
		                           <td><?php echo $row->note; ?></td>
					                        ...
					                        <td>
                                  <a 
                              href="<?php echo base_url(); ?>/account/edit/<?php echo $row->account_id; ?>"> <button type="button"  style=" width:100%;"><i class="fa fa-pencil"></i>
                               </a>
                              </td>
                      

                              <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->account_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button>
                              </td>
                           </tr>
										<?php } ?>
									</tbody>
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/account/delete',
								  method: 'POST',
									data:{vdelv: del_id},
									success:function(data){
										//alert(data);
