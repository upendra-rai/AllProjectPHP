...
// ************Product Sale Report ***********//
//=========********===========********========******========//   
public function product_sale_report($start,$end,$product){
          
       
       
                $this->db->select('*,product_sales.created_at AS row_date');
               $this->db->from('product_sales');
               $this->db->join('product_details','product_details.product_id = product_sales.product_id');
               $this->db->join('product_stock','product_stock.product_stock_id = product_details.product_id');
               $this->db->join('unit','unit.unit_id = product_sales.sale_unit_id');
             $this->db->join('sales','sales.sale_id = product_sales.sale_id');

               //**********Outlet Filter ********//
                 if($this->session->userdata('user_role') != 'master admin' || $this->session->userdata('outlet_id')){
                      $user_outlet = $this->session->userdata('outlet_id');
                      $this->db->where('sales.outlet_id',$user_outlet);
                 }
                 //**********Outlet Filter ********//
                ...
                if ($start != '') {
                  ...
                  if($product !=''){
                    $this->db->where('product_sales.product_id' , $product);
                  }

               $data = $this->db->get();
                return $data->result();
    }
    public function product_searchbar(){
     $this->db->select('*');
...
// ************Sale Report ***********//
//=========********===========********========******========//
  public function getsale($start,$end,$outlet_name,$user_name){
         $this->db->select('*,sales.created_at AS row_date');
         $this->db->from('sales');
         $this->db->join('staff_member','staff_member.staff_id = sales.user_id');
         $this->db->join('customer_details','customer_details.customer_id = sales.customer_id');
         $this->db->join('outlet_details','outlet_details.outlet_id = sales.outlet_id');
               //**********Outlet Filter ********//
                 if($this->session->userdata('user_role') != 'master admin' || $this->session->userdata('outlet_id')){
                      $user_outlet = $this->session->userdata('outlet_id');
                      $this->db->where('sales.outlet_id',$user_outlet);
                ...
                if ($start != '') {
                  ...
                  if($outlet_name !=''){
                    $this->db->where('outlet_details.outlet_id' , $outlet_name);
                  }
                   if($user_name !=''){
                    $this->db->where('staff_member.staff_id' , $user_name);
                  }

               $data = $this->db->get();
                return $data->result();
  }
public function sale_searchbar(){
     $this->db->select('*');
     $this->db->from('outlet_details');
     $data = $this->db->get();
     return $data->result();
    }
    public function sale_searchbar1(){
     $this->db->select('*');
     $this->db->from('staff_member');
     $data = $this->db->get();
     return $data->result();
